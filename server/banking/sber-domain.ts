...
export const SBER_ACCOUNT = '40702810438720035571';
export const SBER_COMPANY = 'ООО «НК АРТЭЛЬ»';
export const SBER_INN = '5050140563';
export const SBER_CONNECTION = 'sber-nk-artel';
...
export const emptySber = (): SberData => ({ version: 1, account: SBER_ACCOUNT, days: [], operations: [] });
...
export function normalizeSberOperation(raw: unknown, day: string): BankOperation {
  ...
  if (ownAccount && ownAccount !== SBER_ACCOUNT) throw new ApiError(502, 'Сбер вернул операцию другого счёта. Предыдущая выписка сохранена.');
  ...
  return {
    id: operationId(SBER_CONNECTION, SBER_ACCOUNT, bankId), connectionId: SBER_CONNECTION, provider: 'sber', bankOperationId: bankId,
    account: SBER_ACCOUNT, statementDate: day, amount: moneyObject(amount, false)!, currency: 'RUB', direction: r.direction === 'CREDIT' ? 'incoming' : 'outgoing',
...
export function parseSberPage(raw: unknown, day: string, page: number): { operations: BankOperation[]; nextPage?: number } {
  ...
  if (links.length) {
    try {
      ...
      if (nextPage !== page + 1 || nextPage > 10000 || url.searchParams.has('accountNumber') && url.searchParams.get('accountNumber') !== SBER_ACCOUNT || url.searchParams.has('statementDate') && url.searchParams.get('statementDate') !== day) throw new Error('Invalid page');
  ...
  return { operations: body.transactions.map(row => normalizeSberOperation(row, day)), nextPage };
}
...
export function validateSber(value: SberData | undefined) {
  if (value === undefined) return;
  if (!value || value.version !== 1 || value.account !== SBER_ACCOUNT || !Array.isArray(value.days) || !Array.isArray(value.operations)) throw new Error('Invalid Sber storage');
...
export function encryptSberTokens(tokens: SberTokens, env: Record<string, string | undefined>): string {
  const iv = randomBytes(12), cipher = createCipheriv('aes-256-gcm', sberEncryptionKey(env), iv);
  cipher.setAAD(Buffer.from(`artel:sber:${SBER_ACCOUNT}:v1`));
  const encrypted = Buffer.concat([cipher.update(JSON.stringify(tokens), 'utf8'), cipher.final()]);
  return ['v1', iv.toString('base64'), cipher.getAuthTag().toString('base64'), encrypted.toString('base64')].join('.');
}
export function decryptSberTokens(encoded: string, env: Record<string, string | undefined>): SberTokens {
  try {
    ...
    decipher.setAAD(Buffer.from(`artel:sber:${SBER_ACCOUNT}:v1`));
