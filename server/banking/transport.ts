...
export function bankConfig(definition: BankDefinition, env: Record<string, string | undefined> = process.env): BankConfig {
  ...
  if (definition.provider === 'sber') {
    for (const [key, label] of [['CLIENT_ID','Client ID'],['CLIENT_SECRET','Client secret'],['REFRESH_TOKEN','Первичная авторизация и refresh token']] as const) {
      if (!env[`${prefix}_${key}`] && !(key.endsWith('_PATH') && env[`${prefix}_${key.replace('_PATH','_BASE64')}`])) missing.push(label);
    }
    const pfx = env[`${prefix}_TLS_PFX_PATH`] || env[`${prefix}_TLS_PFX_BASE64`];
    if (pfx) {
      if (!env[`${prefix}_TLS_PASSPHRASE`]) missing.push('Пароль контейнера TLS');
    } else {
      if (!env[`${prefix}_TLS_CERT_PATH`] && !env[`${prefix}_TLS_CERT_BASE64`]) missing.push('TLS-сертификат');
      if (!env[`${prefix}_TLS_KEY_PATH`] && !env[`${prefix}_TLS_KEY_BASE64`]) missing.push('Закрытый ключ TLS');
    }
...
export async function bankTlsMaterial(config: BankConfig) {
  ...
  try {
    const [cert, key, ca, pfx] = await Promise.all([material('TLS_CERT'), material('TLS_KEY'), material('TLS_CA'), material('TLS_PFX')]);
    if (pfx && (cert || key)) throw new Error('Choose one TLS container format');
    return { cert, key, ca, pfx, passphrase: config.env[`${prefix}_TLS_PASSPHRASE`] };
...
export async function bankToken(config: BankConfig, encrypted: string | undefined, save: (encrypted: string) => Promise<void>, http: BankRequest = bankRequest): Promise<string> {
  ...
  const granted = str(token.scope) ?? str(saved.scope);
  if (saved.scope && (!granted || granted.split(/\s+/).some(scope => !String(saved.scope).split(/\s+/).includes(scope)) || !granted.split(/\s+/).includes('GET_STATEMENT_ACCOUNT'))) throw new ApiError(403, 'СберБизнес изменил права токена. Требуется повторная авторизация.');
  await save(encryptTokens({ access_token: token.access_token, refresh_token: token.refresh_token, expiresAt: Date.now() + Number(token.expires_in) * 1000, ...(granted ? { scope: granted } : {}) }, key, config.definition.id));
