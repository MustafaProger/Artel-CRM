...
export default function SberLayout({ bank, onBack }: { bank: { bankName: string; company: string }; onBack: () => void }) {
  return <>
    <div className="bank-section-heading"><div><button className="bank-back" onClick={onBack}><ArrowLeft size={16}/>Все подключения</button><h2>{bank.bankName} · {bank.company}</h2></div><div className="bank-heading-actions"><button className="button" disabled><Unplug size={16}/>Подключение</button><button className="button primary" disabled><RefreshCw size={16}/>Синхронизировать</button></div></div>
    ...
    <div className="panel bank-ledger">
      ...
      <div className="bank-empty"><Unplug size={30}/><h3>СберБизнес ещё не подключён</h3></div>
