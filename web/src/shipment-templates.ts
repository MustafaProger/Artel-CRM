...
export const shipmentColumns: ShipmentColumn[] = [
  ...
  column('payment_form', 'Форма оплаты', 'operation', 'text', 78),
  column('product', 'Товар', 'operation', 'text', 64),
  column('quantity_tonnes', 'Кол-во, т', 'sale', 'number', 82),
  column('quantity_litres', 'Кол-во, л', 'sale', 'number', 92),
  column('sale_price_per_tonne', 'Цена продажи, т', 'sale', 'number', 100),
  column('sale_price_per_litre', 'цена продажи за л', 'sale', 'number', 96),
  column('customer_amount', 'Сумма покупателя', 'sale', 'number', 118),
  column('unloading_address', 'Адрес выгрузки', 'sale', 'text', 174),
  column('supplier_name', 'Поставщик', 'purchase', 'company', 144),
  column('supplier_inn', 'ИНН Поставщика', 'purchase', 'inn', 110),
  column('purchase_price_unspecified_unit', 'Цена закупа', 'purchase', 'number', 100),
  column('purchase_amount', 'Сумма закупки', 'purchase', 'number', 118),
  column('loading_address', 'Адрес загрузки', 'purchase', 'text', 174),
  column('carrier_name', 'Перевозчик / водитель', 'delivery', 'text', 140),
  column('vehicle_plate', 'Автомобиль / госномер', 'delivery', 'text', 112),
  column('transport_amount', 'Сумма перевозки', 'delivery', 'number', 110),
  column('kvp_source', 'КВП', 'delivery', 'number', 82),
  column('additional_costs', 'Доп Затраты', 'delivery', 'number', 98),
  column('costs_breakdown', 'КВП / Допзатраты', 'delivery', 'text', 174),
  column('profit_source', 'Прибыль', 'settlement', 'number', 110),
  column('paid_amount_source', 'Оплата', 'settlement', 'number', 110),
  column('payment_date', 'Дата оплаты', 'settlement', 'date', 88),
  ...
  column('unlabelled_note', 'Дата из файла', 'settlement', 'date', 88),
]
...
export const azsShipmentColumns: ShipmentColumn[] = [
  ...
  column('payment_form', 'Форма оплаты', 'operation', 'text', 78),
  column('product', 'Товар', 'operation', 'text', 64),
  column('quantity_litres', 'Количество литров', 'sale', 'number', 92),
  column('customer_amount', 'Сумма покупателя', 'sale', 'number', 118),
  column('purchase_amount', 'Сумма поставщика', 'sale', 'number', 118),
  column('sale_price_per_litre', 'Цена продажи за литр', 'sale', 'number', 96),
  column('supplier_name', 'Поставщик', 'purchase', 'company', 144),
  column('supplier_inn', 'ИНН поставщика', 'purchase', 'inn', 110),
  column('kvp_source', 'КВП', 'settlement', 'number', 82),
  column('profit_source', 'Прибыль', 'settlement', 'number', 110),
  column('paid_amount_source', 'Оплата', 'settlement', 'number', 110),
  column('debt_overpayment_source', 'Долг/переплата', 'settlement', 'number', 116),
  column('days_since_shipment', 'Дней с отгрузки', 'settlement', 'number', 80),
