...
export function calculateShipment(input: Record<string, string | null>, rules: CalculationRules, options: {
  ...
  const unresolvedExtras = !!rules.profit && rules.profit !== 'simple' && !!value('additional_costs') && !value('additional_costs')!.isZero()
  if (unresolvedExtras) warnings.push('В исходной формуле нет дополнительных затрат. Уточните их учёт в прибыли.')
  ...
  if (!options.historical || options.recalculate) {
    ...
    else if (sale && purchase && !unresolvedExtras) {
