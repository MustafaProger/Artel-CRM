...
function Stat({label,value,unit,description,icon:Icon,trend}: {label:string;value:string;unit?:string;description?:string;icon:LucideIcon;trend?:number[]}) {return <div className="stat-card"><div className="stat-label">{label}<Icon size={18}/></div><div className="stat-value">{value}<span>{unit}</span></div>{(description || trend) && <div className="stat-bottom">{description && <span>{description}</span>}{trend && <Sparkline values={trend}/>}</div>}</div>}
...
function ExportButton({onClick,disabled=false}: {onClick:()=>void;disabled?:boolean}) {return <button className="button" disabled={disabled} onClick={onClick}><ArrowDownToLine size={16}/>Экспорт CSV</button>}
function Payments({data,period,onPeriodChange,open,notify}: {data:Snapshot;period:string;onPeriodChange:(value:string)=>void;open:(d:Detail)=>void;notify:()=>void}) {
 ...
 const resetFilters=()=>{setQuery('');setDirection('all');onPeriodChange('all');setPage(1)}
 const archiveHeading=<div className="archive-heading"><div><h2>Архив выписок</h2><p>Платежи, сохранённые из файла</p></div>{data.payments.length>0&&<label className="period-control"><CalendarDays size={16}/><select aria-label="Период архива" value={period} onChange={e=>onPeriodChange(e.target.value)}><option value="all">Все месяцы</option>{data.monthly.map(m=><option key={m.month} value={m.month}>{monthName(m.month)}</option>)}</select></label>}</div>
 if(!data.payments.length)return <section className="payment-archive">{archiveHeading}<div className="panel archive-empty"><span className="archive-empty-icon"><FileText size={28}/></span><h3>В архиве пока нет выписок</h3><p>Здесь будут отображаться платежи из файла.<br/>Выписки подключённых банков доступны во вкладке «Банковские подключения».</p></div></section>
 return <section className="payment-archive">{archiveHeading}<div className="payment-stats"><Stat label="Поступления" value={shortNumber(sum(filtered.map(p=>p.incoming)))} unit="₽" icon={ArrowDownLeft}/><Stat label="Списания" value={shortNumber(sum(filtered.map(p=>p.outgoing)))} unit="₽" icon={ArrowUpRight}/><Stat label="Строки выписки" value={number(filtered.length)} icon={Wallet}/></div><section className="panel archive-ledger"><div className="table-toolbar"><SearchField value={query} onChange={setQuery} placeholder="Контрагент или назначение…"/><div className="toolbar-actions"><select aria-label="Направление платежа" className="filter-select" value={direction} onChange={e=>setDirection(e.target.value)}><option value="all">Все операции</option><option value="incoming">Поступления</option><option value="outgoing">Списания</option><option value="undated">Без даты</option></select><ExportButton disabled={!filtered.length} onClick={()=>{downloadCsv('Артель-платежи.csv',['Месяц из файла','Дата','Строка источника','Контрагент','Поступление, ₽','Списание, ₽','Назначение','Доп. поле из файла'],filtered.map(p=>[p.fields.month??null,p.date,p.sourceRow,p.counterparty,p.incoming,p.outgoing,p.purpose,p.fields.unlabelled_extra??null]));notify()}}/></div></div><div className="table-scroll">{filtered.length>0&&<table aria-label="Платежи из выписки"><thead><tr><th data-field="month">Месяц из файла</th><th>Дата / строка</th><th>Контрагент</th><th>Назначение платежа</th><th className="align-right">Поступление, ₽</th><th className="align-right">Списание, ₽</th><th data-field="unlabelled_extra">Доп. поле из файла</th><th/></tr></thead><tbody>{filtered.slice((page-1)*12,page*12).map(p=><tr key={p.id} data-source-row={p.sourceRow}><td data-field="month" className="tabular">{p.fields.month??'—'}</td><td><button className="date-link" onClick={()=>open({kind:'payment',item:p})}>{formatDate(p.date)}<small>#{p.sourceRow}</small></button></td><td><button className="company-text-link" onClick={()=>open({kind:'payment',item:p})}>{p.counterparty||'Не указан'}</button></td><td className="purpose-cell" title={p.purpose||''}>{p.purpose||'—'}</td><td className="align-right tabular positive">{money(p.incoming,2)}</td><td className="align-right tabular">{money(p.outgoing,2)}</td><td data-field="unlabelled_extra">{p.fields.unlabelled_extra??'—'}</td><td><button className="icon-button" aria-label={`Открыть платёж ${p.sourceRow}`} onClick={()=>open({kind:'payment',item:p})}><ChevronRight size={15}/></button></td></tr>)}</tbody></table>}{!filtered.length&&<div className="archive-empty archive-no-results"><Search size={28}/><h3>Нет операций по выбранным условиям</h3><p>Попробуйте другой период, направление или имя контрагента.</p><button className="button" onClick={resetFilters}>Сбросить фильтры</button></div>}</div>{filtered.length>0&&<Pagination page={page} setPage={setPage} total={filtered.length}/>}</section></section>
}
