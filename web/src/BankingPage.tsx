...
export default function BankingPage({ legacy }: { legacy: ReactNode }) {
  ...
  return <section className="banking-page">
    ...
    {source === 'legacy' ? <><p className="bank-notice">Сохранённая XLSX-выписка. Её суммы показаны отдельно: в исходном файле нет достаточных реквизитов для достоверного сопоставления с API. Данные и прежний экспорт сохранены.</p>{legacy}</> : sber ? sber.id === 'sber-nk-artel' ? <SberStatements onBack={() => selectBank('')}/> : <SberLayout bank={sber} onBack={() => selectBank('')}/> : <>
