  ...
  return <section className="banking-page">
    ...
    {source === 'legacy' ? <>{legacy}</> : sber ? <SberStatements key={sber.id} connectionId={sber.id} onBack={() => selectBank('')}/> : <>
      {selectedBank ? <BankConnectionHeader bankName={selectedBank.bankName} company={selectedBank.company} provider={selectedBank.provider} onBack={() => selectBank('')}
        ...
        {periodControls}
        {card && <div className="bank-connection-line"><span className={`bank-state ${card.state}`}>{stateName[card.state]}</span><span>Обновлено: <strong>{time(card.lastSuccessAt)}</strong></span>{card.lastCompletedPeriod && <span>Загружен период: {date(card.lastCompletedPeriod.from)} — {date(card.lastCompletedPeriod.to)}</span>}</div>}
        {data?.scheduleEnabled && <p className="bank-auto-sync"><RefreshCw size={14}/>Автообновление каждые 5 минут</p>}
      </BankConnectionHeader> : <>
        <div className="bank-section-heading"><h2>Банковские подключения</h2><div className="bank-heading-actions">{syncControls}</div></div>
        {periodControls}
      ...
      {loading && !data ? <div className="bank-empty" role="status"><LoaderCircle className="spin"/><h3>Загружаем подключения…</h3></div> : <>
        ...
        {connection && <div className="panel bank-ledger"><div className="bank-ledger-title"><div><h3>Операции по счетам</h3><p>{data?.total ?? 0} операций</p></div><button className="button" disabled={loading || !!error || !data?.total} onClick={() => { setActionError(''); void download(`/api/banking/export?${params}`, 'Артель-банковские-операции.csv').catch(e => setActionError(e.message)) }}><ArrowDownToLine size={16}/>Экспорт CSV</button></div>
...
function PaymentPanel({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="bank-payment-panel" aria-labelledby="bank-payment-title" onCancel={onClose} onClick={e => { if (e.target === dialog.current) { const bounds = dialog.current.getBoundingClientRect(); if (e.clientX < bounds.left || e.clientX > bounds.right || e.clientY < bounds.top || e.clientY > bounds.bottom) onClose() } }}><div className="bank-panel-header"><div><h2 id="bank-payment-title">Платёж {row?.documentNumber ? `№ ${row.documentNumber}` : ''}</h2><p>{bank?.bankName} · {bank?.company}</p></div><button autoFocus className="icon-button" aria-label="Закрыть карточку платежа" onClick={onClose}><X size={21}/></button></div>
    {error && <p className="bank-notice bank-error" role="alert">{error}</p>}{!row ? <div className="bank-empty" role="status">{error ? 'Реквизиты недоступны' : <><LoaderCircle className="spin"/>Загружаем реквизиты…</>}</div> : <div className="bank-panel-body">
      ...
      <details className="bank-raw-details"><summary>Все доступные поля банковского ответа</summary><dl>{rawFields(row.bankData).map(([key, value]) => field(key, value))}</dl></details>
