...
export default function WorkPage() {
  ...
  const userName = (id: string) => [...(data?.users ?? []), ...(data?.historicalUsers ?? [])].find(user => user.id === id)?.name ?? 'Сотрудник недоступен';
...
function WorkEditor({ editor, users, historicalUsers, companies, actor, onClose, onSaved }: { editor: Editor; users: AccountUser[]; historicalUsers: Pick<AccountUser, 'id' | 'name'>[]; companies: Company[]; actor: AccountUser; onClose: () => void; onSaved: (action: SavedAction, entry?: AnyWorkEntry) => void }) {
  const dialog = useRef<HTMLDialogElement>(null), inFlight = useRef(false);
  const userName = (id: string) => [...users, ...historicalUsers].find(user => user.id === id)?.name ?? 'Сотрудник недоступен';
  ...
  const save = async (method: 'POST' | 'PATCH' | 'DELETE', nextArchived?: boolean) => {
    ...
    try {
      ...
      if (method !== 'DELETE') {
        const reminderAt = fields.reminderAt === localInputTime(task?.reminderAt)
          ? task?.reminderAt ?? null
          : fields.reminderAt ? new Date(fields.reminderAt).toISOString() : null;
  ...
  return <dialog ref={dialog} className="detail-dialog work-editor" aria-labelledby="work-editor-title" onCancel={event => { event.preventDefault(); close(); }}><form onSubmit={event => { event.preventDefault(); void save(entry ? 'PATCH' : 'POST'); }}>
    ...
    <div className="work-editor-body">
      ...
      <div className="work-field-grid"><label className="work-field"><span>Исполнитель *</span><select aria-label="Исполнитель" value={fields.assigneeId} required disabled={saving || !entry && actor.role === 'manager'} onChange={event => update('assigneeId', event.target.value)}>{!users.some(user => user.id === fields.assigneeId) && <option value={fields.assigneeId} disabled>{userName(fields.assigneeId)} · доступ отключён</option>}{users.map(user => <option value={user.id} key={user.id}>{user.name}{user.id === actor.id ? ' (я)' : ''}</option>)}</select></label>{editor.kind === 'tasks' && <label className="work-field"><span>Состояние</span><select aria-label="Состояние задачи" value={fields.status} disabled={saving} onChange={event => update('status', event.target.value)}>{workStatuses.map(status => <option key={status.id} value={status.id}>{status.name}</option>)}</select></label>}
      ...
      {editor.kind !== 'notes' && <p className="work-hint">При включённых уведомлениях напоминание придёт исполнителю на устройство. Укажите время в часовом поясе вашего устройства.</p>}
      {editor.kind !== 'notes' && <section className="work-comments"><h3>Комментарии</h3>{record?.comments?.map(comment => <article key={comment.id}><small>{userName(comment.authorId)} · {dateLabel(comment.createdAt)}</small><p>{comment.text}</p></article>)}{input('Новый комментарий', 'comment', { multiline: true })}</section>}
      ...
      {entry && <p className="work-hint">Изменено: {dateLabel(entry.updatedAt)} · {userName(entry.updatedBy)}</p>}
      {error && <p className="shipment-error" role="alert">{error}</p>}
