...
export default function SberStatements({ onBack }: { onBack: () => void }) {
  ...
  return <div className="sber-statements">
    <div className="bank-section-heading">
      <div><button className="bank-back" onClick={onBack}><ArrowLeft size={16}/>Все подключения</button><h2>СберБизнес · {data?.company ?? 'ООО «НК АРТЭЛЬ»'}</h2><p>Выписки по расчётному счёту · только просмотр</p></div>
    ...
    <div className="bank-connection-line"><span className={`bank-state ${busy ? 'syncing' : syncError ? 'error' : data?.missing.length ? 'not_configured' : data?.lastSuccessAt ? 'connected' : 'ready'}`}>{busy ? 'Синхронизация' : syncError ? 'Обновление не завершено' : data?.missing.length ? 'Доступ не настроен' : data?.lastSuccessAt ? 'Данные получены из банка' : 'Ожидает первой загрузки'}</span><span>Последняя успешная синхронизация: <strong>{displayTimestamp(data?.lastSuccessAt)}</strong></span>{data?.lastCompletedPeriod && <span>Загружен период: {displayDate(data.lastCompletedPeriod.from)} — {displayDate(data.lastCompletedPeriod.to)}</span>}</div>
    ...
    {loading && !data ? <div className="bank-empty" role="status"><LoaderCircle className="spin"/><h3>Загружаем сохранённую выписку…</h3></div> : data && <>
      <div className="panel bank-ledger sber-summary" aria-busy={loading}>
        <div className="bank-ledger-title"><div><h3>Выписки по дням</h3><p>{displayDate(dataPeriod.from)} — {displayDate(dataPeriod.to)} · выберите день для просмотра операций</p></div><label className="sber-zero-toggle"><input type="checkbox" checked={hideZero} onChange={event => setHideZero(event.target.checked)}/>Скрыть дни без оборотов</label></div>
        ...
        <p className="sber-summary-note">Итоги предоставлены банком. «Не передано» означает, что значения нет в ответе банка.{hideZero ? ' Скрываются только дни с подтверждёнными нулевыми поступлениями и списаниями.' : ''}</p>
      </div>
      <div className="panel bank-ledger" id="sber-operations" aria-busy={loading}>
        ...
        <p className="sber-table-footer">Выберите операцию, чтобы увидеть полное назначение и реквизиты. Данные выписки не изменяют таблицу отгрузок.</p>
      </div>
    </>}
...
function OperationDetails({ initial, available, onUpdate, onClose }: { initial: SberOperation; available: boolean; onUpdate: (operation: SberOperation) => void; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="bank-payment-panel sber-payment-panel" aria-labelledby="sber-payment-title" onCancel={onClose} onClick={event => { if (event.target === dialog.current) { const bounds = dialog.current.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) onClose() } }}>
    <div className="bank-panel-header"><div><span className="bank-overline">ОПЕРАЦИЯ СБЕРБИЗНЕС</span><h2 id="sber-payment-title">{row.documentNumber ? `Документ № ${row.documentNumber}` : 'Подробности операции'}</h2><p>ООО «НК АРТЭЛЬ» · {displayDate(row.statementDate)}</p></div><button autoFocus className="icon-button" aria-label="Закрыть подробности операции" onClick={onClose}><X size={21}/></button></div>
    <div className="bank-panel-body">
      ...
      <details className="bank-raw-details"><summary>Все поля, полученные от банка</summary><p>Дополнительные реквизиты и сведения о документе в исходном виде.</p><dl>{rawFields(row.bankData).map(([key, value]) => field(key, value))}</dl></details>
