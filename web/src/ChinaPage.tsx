...
export default function ChinaPage({ canManage }: { canManage: boolean }) {
  ...
  return <section className="panel china-page"><header className="china-heading"><span className="china-period">За всё время</span></header>
    ...
    {!canManage ? <p className="china-hint">Учёт доступен директору и администратору в рамках действующих ролей.</p> : <>
      ...
      <div className="china-table-scroll" tabIndex={0} role="region" aria-label="Таблица Китая с горизонтальной прокруткой"><table className="china-table"><thead><tr><th>Дата</th><th title="Количество литров">Литры</th><th title="Сумма заправок">Заправки</th><th>Поступления</th></tr></thead><tbody>{days.map(date => { const day = data!.china.days.find(day => day.date === date); const payments = data!.china.payments.filter(payment => payment.date === date); return <tr key={date}><td>{day ? <button className="china-day-link" aria-label={`Открыть день ${date}`} onClick={() => setEditor({ kind: 'days', day })}>{<ChinaDate date={date}/>}</button> : <ChinaDate date={date}/>}</td><td>{day ? <strong>{number(sum(day.fuels.map(fuel => fuel.litres)))}</strong> : '—'}</td><td>{day ? <><strong>{number(sum(day.fuels.map(fuel => fuel.amount)))}</strong><details className="china-details"><summary>Заправки · {day.fuels.length}</summary>{day.fuels.map((fuel, index) => <div key={index}>{data!.suppliers.find(supplier => supplier.id === fuel.supplierId)?.name ?? 'Поставщик недоступен'}<small>{number(fuel.litres)} л · {number(fuel.amount)}</small></div>)}</details></> : '—'}</td><td className="china-incoming">{payments.length ? <><strong>+{number(sum(payments.map(payment => payment.amount)))}</strong><details className="china-details"><summary>Платежи · {payments.length}</summary>{payments.map(payment => <div key={payment.id}>+{number(payment.amount)}</div>)}</details></> : '—'}</td></tr>; })}</tbody></table></div>
