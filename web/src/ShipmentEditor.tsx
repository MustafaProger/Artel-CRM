...
function LegacyShipmentEditor({ shipment, companies, directories, defaultPaymentForm = 'б/нал', onClose, onSaved }: ShipmentEditorProps) {
  ...
  const companyEntries=(current:string)=>companies.filter(c=>!c.directoryArchived || c.id===current).map(c=>({id:c.id,name:c.name,detail:c.inn?`ИНН ${c.inn}`:'ИНН не указан в справочнике'}))
