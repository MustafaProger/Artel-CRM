...
function LegacyShipmentEditor({ shipment, companies, directories, defaultPaymentForm = 'б/нал', onClose, onSaved }: ShipmentEditorProps) {
  ...
  const companyEntries=(role:'customer'|'supplier',current:string)=>companies.filter(c=>(!c.directoryArchived && c.roles.includes(role)) || c.id===current).map(c=>({id:c.id,name:c.name,detail:c.inn?`ИНН ${c.inn}`:'ИНН не указан в справочнике'}))
