...
import ShipmentColumnResize, { useShipmentColumnWidths } from './ShipmentColumnResize'
...
export default function ShipmentsPage({onOpenMenu,menuOpen,data,period,onPeriodChange,onChanged,onOpenCompany,initialQuery = '',canDelete = true,canReadDirectories = true}: {
  onOpenMenu: () => void; menuOpen: boolean; data: Snapshot; period: string; onPeriodChange: (period:string)=>void; onChanged: () => void; onOpenCompany: (company: Company) => void; initialQuery?: string; canDelete?: boolean; canReadDirectories?: boolean
}) {
  ...
  const { columns, enabled: canResizeColumns, resize: resizeColumn, reset: resetColumnWidths, customized: customizedWidths } = useShipmentColumnWidths(`${shipmentType}:${shipmentType === 'azs' ? azsFormat : template}`, chosenTemplate.columns)
  ...
  return <div className="shipments-workspace">
    <section className="panel shipment-panel" aria-label="Операции отгрузки">
      <div className="shipment-toolbar"><div className="shipment-title"><button className="icon-button shipment-menu" aria-label="Открыть меню" title="Меню разделов" aria-controls="app-navigation" aria-expanded={menuOpen} onClick={onOpenMenu}><Menu size={21}/></button><h1>Отгрузки</h1></div><label className="shipment-search"><Search size={17}/><input aria-label="Поиск отгрузок" placeholder="Поиск по всем колонкам…" value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<button className="icon-button" aria-label="Очистить поиск" onClick={()=>setQuery('')}><X size={16}/></button>}</label><button className="button primary" onClick={()=>setEditor({shipment:null})}><Plus size={17}/>Добавить отгрузку</button>{canReadDirectories && <a className="button" href="#directories"><BookOpen size={17}/>Справочники</a>}</div>
