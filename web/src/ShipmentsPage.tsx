  ...
  return <div className="shipments-workspace">
    <section className="panel shipment-panel" aria-label="Операции отгрузки">
      ...
      <div ref={scroller} className="shipment-grid-scroll" data-testid="shipments-scroll" tabIndex={0} aria-label="Таблица отгрузок с горизонтальной и вертикальной прокруткой" onScroll={event => {
        ...
        <table className="shipment-grid" style={{width:columns.reduce((total,column) => total + column.width,0) + (showActions ? 108 : 0)}} aria-rowcount={total + 2} aria-colcount={columns.length + extraColumns}>
          <colgroup>{columns.map(column => <col key={column.key} style={{width:column.width}}/>)}{showActions && <col style={{width:108}}/>}</colgroup>
