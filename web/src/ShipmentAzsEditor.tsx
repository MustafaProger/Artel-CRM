...
export default function ShipmentAzsEditor({shipment,companies,directories,defaultPaymentForm='б/нал',onClose,onSaved}:ShipmentEditorProps) {
  ...
  const companyEntries=(role:string,current:string)=>companies.filter(company=>(!company.directoryArchived && company.roles.includes(role)||company.id===current) && (role !== 'customer' || availableShipmentCustomer(directories, company.id, current))).map(company=>({id:company.id,name:company.name,detail:company.inn?`ИНН ${company.inn}`:'ИНН не указан'}))
