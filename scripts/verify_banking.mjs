...
import { accountNumber, dollarAccount, fixtureDay, fixtureEnvironment, tbankRow } from '../tests/banking-fixtures.ts';
...
try {
  ...
  for (const company of ['НК АРТЕЛЬ', 'АРТЕЛЬ']) {
    ...
    assert.deepEqual(bankRequests, [], 'Sber layout must not fetch, poll or continue bank jobs');
    await page.clock.resume();
    for (const width of [1440, 390, 320]) {
      await page.setViewportSize({ width, height: 1000 });
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), `Sber layout overflow ${width}`);
      await page.screenshot({ path: resolve(output, `sber-layout-${company}-${width}.png`), fullPage: true });
    }
    await page.setViewportSize({ width: 1440, height: 1000 });
    const layoutAxe = await new AxeBuilder({ page }).include('.banking-page').withTags(['wcag2a','wcag2aa']).analyze();
    assert.equal(layoutAxe.violations.length, 0, 'Sber layout accessibility');
  ...
  await expect(page.locator('.bank-parties')).toContainText('Клиент тестовый');
  await page.getByRole('button', { name: 'Копировать Плательщик · ИНН', exact: true }).click();
  assert.equal(await page.evaluate(() => navigator.clipboard.readText()), '7812345678');
  const fileEvent = page.waitForEvent('download'); await page.getByRole('button', { name: 'Печатная форма банка', exact: true }).click();
  const pdf = await fileEvent; assert.ok((await readFile(await pdf.path())).toString().startsWith('%PDF-'));
